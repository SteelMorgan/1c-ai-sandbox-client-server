...
ensure_vrd_features() {
  ...
  if grep -qE '<httpServices\b' "${vrd_path}" 2>/dev/null; then
    # Some platform builds do NOT accept extra attributes (e.g. rootUrl=...),
    # so keep it minimal and remove any unknown/legacy attributes if present.
    perl -0777 -i -pe 's#\s+rootUrl="[^"]*"# #g' "${vrd_path}" || true
    # Fix potential missing whitespace between attributes (best-effort).
    perl -0777 -i -pe 's#(<httpServices[^>]*")(?=publishExtensionsByDefault)#$1 #g' "${vrd_path}" || true
    if ! grep -qE '<httpServices[^>]*\bpublishExtensionsByDefault="' "${vrd_path}" 2>/dev/null; then
      perl -0777 -i -pe 's#<httpServices([^>]*)/>#<httpServices$1 publishExtensionsByDefault="true"/>#g' "${vrd_path}" || true
      changed=1
    fi
  else
...
case "${action}" in
  publish|update)
    ...
    set +e
    # If a VRD already exists (e.g. generated by Configurator wizard with explicit HTTP services),
    # publish using -descriptor to preserve its contents.
    vrd_path="${pub_dir}/default.vrd"
    if [[ -f "${vrd_path}" ]]; then
