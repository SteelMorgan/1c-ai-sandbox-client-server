...
install_from_run() {
    ...
    case "$installer_type" in
      server)
        # server_admin: admin tooling (e.g. rac) required for automation
        run_components="server,server_admin,ws,config_storage_server,$nls_install"
        ;;
      server_crs) 
        # server_admin: admin tooling (e.g. rac) required for automation
        run_components="server,server_admin,ws,config_storage_server,$nls_install"
        ;;
      client)
        # Install full client + keep server tools (ibcmd/ibsrv) for automation
        # server_admin is required for ibcmd (admin tools)
        run_components="server,server_admin,client_full,$nls_install"
        ;;
      client32)
